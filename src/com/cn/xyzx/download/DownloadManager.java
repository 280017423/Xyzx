...
public class DownloadManager {
	...
	public boolean hasDownLoaded(String url) {
		for (int i = 0; i < downloadInfoList.size(); i++) {
			...
			String existUrl = downloadInfoList.get(i).getDownloadUrl();
			if (url.equals(existUrl) && HttpHandler.State.SUCCESS == info.getState()) {
				return true;
			}
		}
		return false;
	...
	public int addNewDownload(String url, String fileName, String target, boolean autoResume, boolean autoRename,
			final RequestCallBack<File> callback) throws DbException {
		for (int i = 0; i < downloadInfoList.size(); i++) {
			String existUrl = downloadInfoList.get(i).getDownloadUrl();
			if (null == url || url.equals(existUrl)) {
				return 0;
			}
		}
