...
public class DownloadService extends Service {
	 ...
	 * **/
	public void reStartDownload(String musicName) {
		if (mDownloader.isdownloading()) {
			return;
		}
		int threadCount = 1;// 定义线程数
		String downPath = AppConstant.NetworkConstant.downPath + musicName;// 下载地址
		String savePath = AppConstant.NetworkConstant.savePath;// 保存地址
		mDownloader = mDownloadersMap.get(downPath);
		if (mDownloader == null) {
			mDownloader = new Downloader(downPath, savePath, musicName, threadCount, this, mHandler);
			mDownloadersMap.put(downPath, mDownloader);// 创建完一个新的下载器,必须把它加入到下载器集合里去
		}
		LoadInfoModel loadInfo = mDownloader.getDownloaderInfors();
		mCompleteSizes.put(downPath, loadInfo.getComplete());
	 ...
	 * **/
	public void switchState(String musicName) {
		String url = AppConstant.NetworkConstant.downPath + musicName;
		Downloader downloader = mDownloadersMap.get(url);
		if (downloader != null) {
			...
			if (downloader.isdownloading()) {
				...
				this.reStartDownload(musicName);
			}
		} else {
			// 如果downloaders中没有url的数据,肯定是处于暂停状态，直接开始下载
			reStartDownload(musicName);
		}
	}
